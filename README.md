...
- 提供浏览器版 GUI 和手动启动的本地 helper，用于配置检查、helper 状态确认、配置保存、Keychain 密钥管理、受控真实备份执行、恢复预案、备份历史和安全边界验证。
...
helper 默认只监听 `127.0.0.1:37371`。GUI 顶部会显示 helper 未确认、检查中、在线或离线状态；helper 离线时，加载/保存配置、Keychain 密钥和真实历史按钮会暂时禁用，避免误操作。GUI 选择 `HTTP 助手` 后，可以执行环境检查、加载/保存配置、保存/删除 Keychain 密钥、读取真实备份历史、受控真实备份、恢复预案和隔离的计划校验。真实备份需要先确认摘要，再点击 `执行真实备份`，成功后会自动刷新 helper 备份历史。恢复预案会运行 `codexrestore --plan`，不会执行真实恢复。
