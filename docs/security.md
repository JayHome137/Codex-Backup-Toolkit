...
桌面 bridge 只开放 `helper_status`、`helper_start`、`helper_stop`、`helper_request` 和 `open_path`。`helper_request` 只允许 `/health`、`/config`、`/secret`、`/history` 和 `/run` 这组 helper API，不提供任意 shell 执行入口。
...
真实备份入口需要先确认目标端、加密状态、保留策略和 helper 状态摘要，然后才会启用 `执行真实备份`。执行成功后，GUI 会自动刷新 helper 备份历史并显示最新备份结果。这个流程只发送结构化 `backup` action，不会开放真实恢复、安装、卸载或自动化修改。
