...
0.24.0 起，GUI `安装` 页会展示安装落地验收清单，把 DMG 校验、首次打开、桌面运行时、目标端检查、首次备份验收和恢复边界串联起来。该清单只做状态展示和页面跳转，不执行真实恢复，不下载或安装文件，不安装、卸载、加载或修改已有定时任务。

0.25.0 起，GUI `概览` 页和 `引导` 页会展示首次真实使用路径，把安装验收、目标端配置、doctor 检查、手动确认备份、首次备份验收和恢复边界串联起来。该路径只做状态展示、页面跳转和已有只读 doctor 检查，不执行真实恢复，不下载或安装文件，不安装、卸载、加载或修改已有定时任务，真实备份仍需要用户在概览页手动确认。

0.26.0 起，GUI `概览` 页和 `健康` 页会展示日常使用状态，把首次真实使用路径、最近备份、健康度和只读自动化状态聚合为日常可用、需要关注或有阻断项。该状态只读取已有 GUI 计算结果和 helper 历史，不重新扫描归档内容，不新增 helper API；相关按钮只跳转页面或刷新现有健康信息，不执行真实恢复，不安装、卸载、加载或修改已有定时任务，真实备份仍需要用户手动确认。

0.33.0 起，GUI `诊断` 页会聚合 macOS 桌面成熟度、helper 运行状态、内置 toolkit、配置路径、历史路径、日志路径、首次备份证明和发布 smoke 状态。该页面只读取现有状态和路径，不会安装、卸载、加载或卸载 `launchd`，不会执行真实恢复，也不会创建或删除备份归档。

0.34.0 起，GUI `诊断` 页会展示 `建议修复路径`。这些建议只会跳转到概览、设置、计划页，或刷新 `desktop_diagnostics` 这类只读信息；不会安装、卸载、加载或修改 `launchd`，不会执行真实恢复，不会接管外部 helper，也不会绕过真实备份的手动确认。

0.35.0 起，GUI `概览` 页会展示 `首次打开推荐`。推荐动作只会跳转到 GUI 页面或运行现有只读 `codexbackup --doctor` 检查；不会安装、卸载、加载或修改 `launchd`，不会修改已有真实定时备份任务，不会执行真实恢复，也不会绕过真实备份的手动确认。

0.35.1 起，macOS 本地安装 smoke 脚本只检查已安装 App 的可执行文件、内置 helper/scripts 资源、短暂启动和退出后的端口残留；不会安装、卸载、加载或修改 `launchd`，不会修改已有真实定时备份任务，也不会执行真实恢复。Windows installer smoke 可以通过 `CODEXBACKUP_WINDOWS_INSTALLER_DIR` 检查下载后的 CI artifact，但仍只做文件存在性和非空检查。

0.35.2 起，helper 生成结构化 action 命令和 Keychain 命令时，会对来自 GUI/API 的字符串值使用安全 shell 引用，覆盖备份路径、WebDAV URL、SMB 账号、rclone remote、恢复归档路径、age identity、Keychain service/account/secret 等字段。隔离测试会用真实 shell executor 验证 `$(...)` 这类命令替换不会被执行。

0.36.0 起，GUI 做产品降噪：主导航只保留概览、备份、存储位置、恢复、记录和设置，首启引导、安装验证、健康检查、macOS 诊断和定时备份状态移动到设置页高级入口。这个改动只调整信息架构和用户文案，不改变 helper allowlist、Tauri bridge 权限、真实备份确认规则、恢复预案边界或 launchd 只读边界。

0.33.0 起新增 `tests/test-macos-release-smoke.sh`。它只读检查当前 `.app/.dmg`、sha256、图标和打包内置 helper/scripts/config/examples 资源，不启动 App，不连接 helper，不加载系统任务，不触碰已有真实定时备份任务。
...
GUI 顶部会显示本机服务未确认、检查中、在线或离线状态。服务离线时，配置加载/保存、Keychain 密钥操作和真实历史刷新按钮会禁用；后续健康检查成功后，这些按钮会恢复可用。
