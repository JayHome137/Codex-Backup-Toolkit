...
- [ ] 启动 `node helper/server.mjs`，打开 GUI，选择 `开发连接`，确认本机服务显示在线；验证后停止该进程。
- [ ] 在 GUI 备份页确认真实备份必须先点击 `确认备份内容` 才能点击 `立即备份`。
- [ ] 在 GUI 目标端页确认可以看到 `加载配置`、`保存配置`，SMB/WebDAV 下可以看到 Keychain 密钥面板。
- [ ] 在 GUI 日志页确认可以看到 `刷新历史` 和 `备份历史`。
...
- [ ] 在 GUI 设置页确认 `高级诊断入口` 包含首启引导、安装验证、健康检查、macOS 诊断和定时备份状态。
- [ ] 通过设置页进入健康检查，确认可以看到健康度评分、检查项、最近备份摘要、归档路径和建议动作。
- [ ] 通过设置页进入健康检查，确认 `刷新健康状态` 只读取备份历史和自动化状态，不执行备份、恢复、安装、卸载或修改任务。
- [ ] 通过设置页进入首启引导，确认可以看到桌面环境、目标端、环境检查、本机服务、备份证明和恢复安全边界步骤。
...
- [ ] 通过设置页进入 macOS 诊断，确认 `macOS 诊断中心` 展示桌面成熟度、本机服务、内置资源、配置/历史/日志路径、首次备份证明和 macOS release smoke 状态。
- [ ] 确认 macOS 诊断只提供刷新诊断、打开设置和打开日志，不提供安装任务、卸载任务、加载任务、卸载任务或执行真实恢复按钮。
- [ ] 确认主导航只显示 `概览`、`备份`、`存储位置`、`恢复`、`记录` 和 `设置`，高级诊断不再占据日常主导航。
- [ ] 确认未设置 `CODEX_BACKUP_SYNC_ENABLED=1` 时，定时脚本仍执行普通备份，不改变既有定时任务行为。
- [ ] 打开桌面 App，确认本机服务可以自动启动或在 `设置` 页一键启动。
- [ ] 确认 `设置` 页 `刷新诊断` 能显示本机服务、内置资源、配置、历史、日志和版本信息。
- [ ] 确认 App 托管本机服务的输出写入 `desktop-helper.out.log` 和 `desktop-helper.err.log`。
- [ ] 退出桌面 App 后，确认由 App 启动的托管本机服务没有在 `37371` 残留。
- [ ] 手动启动外部本机服务后再打开桌面 App，确认显示外部服务在线，退出 App 不停止外部服务。
- [ ] 确认 `设置` 页可以打开配置目录、日志目录和 toolkit 目录。
- [ ] 在 GUI `桌面 App` 模式和 `开发连接` 模式下，确认真实备份都必须先确认备份内容。
...
- [ ] 确认本机服务或 GUI 只开放真实备份执行，仍阻止安装、卸载、恢复和 status。
- [ ] 确认加密备份命令没有 age 收件人或收件人文件时会被本机服务阻止。
- [ ] 确认 macOS 诊断的建议修复路径只做页面跳转或只读刷新，不安装、卸载、加载或修改 `launchd`，不执行真实恢复。
...
- [ ] Release 标题使用中文，例如：`Codex-Backup-toolkit v0.36.0` 可以保留项目名和版本号，但说明正文只写中文。
...
- [ ] 说明 GUI 新增首启引导页，串联桌面环境、目标端、doctor、本机服务、备份证明和恢复边界。
...
- [ ] 说明 GUI 已接入配置保存、Keychain 密钥操作和备份历史读取。
- [ ] 说明 GUI 增加本机服务在线/离线状态、离线禁用动作和更清楚的加载/错误反馈。
- [ ] 说明 GUI 真实备份需要先确认摘要，成功后会自动刷新备份历史。
