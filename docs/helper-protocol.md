...
- `sync`: `./scripts/codexbackup.sh --sync-check --target <target>` 或 `./scripts/codexbackup.sh --sync-local-authoritative --target <target>`，当前只允许 `local` 和 `smb`
...
0.4.0 起，GUI 优先发送结构化 `action`，helper 由 action 生成命令。这样浏览器不再需要把完整 shell 命令当成执行意图。加密备份和一致性同步 action 必须包含 `ageRecipient` 或 `ageRecipientFile`。
...
读取 helper 记录的备份历史。成功执行结构化 backup action 后，helper 会记录目标端、状态、时间、退出码和归档路径。成功执行结构化 sync action 时，只有本次确实创建了新备份才会记录历史。默认路径：
...
- `kind`: `doctor`、`backup`、`sync`、`restorePlan` 或 `validate`。
