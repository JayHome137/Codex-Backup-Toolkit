...
describe('App', () => {
  it('shows WebDAV storage fields without exposing command previews', () => {
    ...
    expect(screen.getByText(/地址填 WebDAV 根目录/i)).toBeInTheDocument();
    expect(screen.getByText(/备份归档会保存到 codex-backups/i)).toBeInTheDocument();
  ...
  it('keeps storage choices focused on local and WebDAV by default', () => {
    render(<App />);

    clickNav(/存储位置/i);

    expect(screen.getByRole('button', { name: /^本地目录$/i })).toBeInTheDocument();
    expect(screen.getByRole('button', { name: /^WebDAV$/i })).toBeInTheDocument();
    expect(screen.queryByRole('button', { name: /SMB/i })).not.toBeInTheDocument();
  ...
  it('shows storage checks with common failure guidance without validation commands', async () => {
    ...
    expect(screen.getByText('手动创建备份文件夹')).toBeInTheDocument();
    expect(screen.getByText('评估加密')).toBeInTheDocument();
    expect(screen.queryByText('401 或 403')).not.toBeInTheDocument();
  });
  ...
  it('detects local content even when helper config and history are offline', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('0 / 5 项已发现')).toBeInTheDocument();
  ...
  it('detects desktop local backup content through Tauri without helper enrichment', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('1 / 2 项已发现')).toBeInTheDocument();
  ...
  it('shows post-install release verification without adding updater, signing, or real restore controls', async () => {
    ...
    expect(screen.getByText('CodexBackup_0.36.9_aarch64.dmg')).toBeInTheDocument();
    expect(screen.getByText('CodexBackup_0.36.9_aarch64.dmg.sha256')).toBeInTheDocument();
    expect(screen.getByText('shasum -a 256 -c CodexBackup_0.36.9_aarch64.dmg.sha256')).toBeInTheDocument();
    ...
    await waitFor(() => {
      expect(navigator.clipboard.writeText).toHaveBeenCalledWith('shasum -a 256 -c CodexBackup_0.36.9_aarch64.dmg.sha256');
    });
  });
  ...
  it('previews latest restore state without exposing restore commands', () => {
    ...
    expect(screen.queryByText(/CODEX_BACKUP_TARGET=webdav/)).not.toBeInTheDocument();
  ...
  it('normalizes older hidden storage targets to the local GUI entry', async () => {
    const fetchMock = vi.fn(async () => jsonResponse({
      schema: 'codex-backup-helper.v1',
      version: 1,
      status: 'ok',
      config: { ...baseConfigResponse(), target: 'rclone' },
    }));
    vi.stubGlobal('fetch', fetchMock);
    ...
    await waitFor(() => {
      expect(screen.getByLabelText('本地备份目录')).toBeInTheDocument();
    });
    expect(screen.queryByRole('button', { name: /rclone/i })).not.toBeInTheDocument();
  });
  ...
  it('saves a typed WebDAV password before running the connection check', async () => {
    ...
    const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
      ...
      if (String(input).endsWith('/run')) {
        return jsonResponse({
          ...
          status: 'ok',
          stdout: 'codexbackup doctor\nTarget: webdav\nok: WebDAV target reachable\nok: WebDAV backup folder reachable\nok: WebDAV backup folder writable\nDoctor passed.',
          stderr: '',
          exitCode: 0,
          audit: {
            commandKind: 'doctor',
    ...
    fireEvent.click(screen.getByRole('button', { name: /检测 WebDAV 连接/i }));
  ...
  it('shows helper lifecycle controls and product paths in Settings', () => {
    ...
    expect(screen.getByText('应用设置')).toBeInTheDocument();
    expect(screen.getAllByText('0.36.9').length).toBeGreaterThan(0);
