...
- 修复 WebDAV 检测和真实备份链路不一致的问题：连接检测现在会检查根目录、`codex-backups` 备份子目录，并执行小文件写入后删除的可写性验证。
- WebDAV 表单和设置向导明确说明：服务器地址填写 WebDAV 根目录，备份归档会保存到其中的 `codex-backups` 文件夹。
- 真实 WebDAV 备份改为以归档上传为主链路；恢复工具包上传失败时只给出警告，不再把已经上传成功的备份判为失败。
- 新增 WebDAV 真实上传隔离测试，并接入 CI，覆盖归档、manifest 和恢复工具包上传降级场景。

## 0.36.8 - 2026-06-09

- WebDAV 设置页参考 CCSwitch 的设置流做进一步收敛：前台保留地址、账号、密码、连接检测和保存配置，不再单独暴露密码保存按钮。
- 新增 `测试并保存 WebDAV` 主流程：先保存临时输入的密码到系统安全存储，再执行只读连接检测，检测通过后保存配置。
- WebDAV 表单改为更紧凑的连接信息布局，并保留目标文件夹需手动创建的提醒和折叠地址示例。
- 存储位置页把检查说明改成 `连接检测` 摘要，配置检查、Keychain、密钥等内部实现信息不再出现在普通用户主路径。

## 0.36.7 - 2026-06-09

- 继续精简普通用户界面：存储位置页主入口只保留 `本地目录` 和 `WebDAV`，SMB/rclone 等能力保留在底层脚本和文档中，不再作为当前 GUI 主路径展示。
- WebDAV 主流程新增密码或应用专用密码输入，可通过本机服务写入 macOS Keychain；保存配置仍不会写入密码。
- WebDAV 保存位置检查升级为只读连接检测：使用 Keychain 或临时环境变量中的密码执行 PROPFIND，能识别认证失败、目标文件夹缺失和不可访问等问题。
- WebDAV 表单明确提醒：目标文件夹需要先在 WebDAV 服务端手动创建，连接检测不会自动创建根目录。
- 本机内容检测的详细路径列表改成紧凑单行布局，路径默认省略显示，避免长路径折行撑坏界面。

## 0.36.6 - 2026-06-09

- 修复桌面 App 一键检测可能退回浏览器 fallback 的问题：前端改用 Tauri 官方 API 调用桌面命令，不再依赖全局注入。
- 本机内容检测范围与真实备份脚本对齐，去掉重复的 `OpenAI/Codex` 子目录展示。
- 概览里的本机内容检测进一步压缩为摘要优先，详细路径默认收起，减少普通用户可见信息量。
- 存储位置页默认只突出本地目录和 WebDAV，高级目标、密钥、配置检查和加密/同步设置收进折叠区域。

## 0.36.5 - 2026-06-09

- 修复概览页 `一键检测本机内容` 的产品逻辑：检测只读取本机可备份数据路径，不再依赖本机服务里的配置和历史记录。
- `可备份内容` 和 `未发现内容` 只统计真实要打包的数据位置，不再把 CodexBackup 自己的配置、历史和日志目录算进去。
- 检测失败提示只保留路径读取相关问题，避免普通用户看到后台服务离线导致的误导性警告。

## 0.36.4 - 2026-06-09

- 主界面继续降噪：移除顶部本机服务连接条，服务状态保留在设置和高级诊断里。
- 概览页新增更靠前的 `备份保存位置` 模块，用户可直接修改或检查保存位置。
- `本机内容检测` 默认只展示摘要，完整路径改为手动展开查看，避免路径列表挤压界面。
- 存储位置页把设置向导收敛为 `保存位置检查`，不再默认展示验证命令、环境变量和脚本命令预览。
- 备份、存储位置、恢复等主流程页面不再展示命令预览；命令级信息保留在底层代码和高级诊断语境里。
- GUI/Tauri 版本升至 `0.36.4`。

## 0.36.3 - 2026-06-08

- 修复概览页 `一键检测本机内容`：桌面 App 现在通过 Tauri 只读读取本机 Codex 相关路径，不再把本机服务配置/历史读取作为核心前置条件。
- 检测结果新增路径存在状态，能直接显示本机数据目录、应用配置、历史和日志路径是否存在。
- 本机服务配置和历史改为附加信息：读取失败只显示提示，不影响本机内容检测完成。
- GUI/Tauri 版本升至 `0.36.3`。

## 0.36.2 - 2026-06-08

- 概览页进一步收敛为状态总览：只展示存储位置、本机服务、完整性、最近备份和下一步入口。
- 备份页只保留真实备份和本地为准同步操作，不再重复展示归档、sha256 和 manifest 详情。
- 详细备份结果统一放在记录页，减少概览、备份和记录之间的信息重复。
- GUI/Tauri 版本升至 `0.36.2`。

## 0.36.1 - 2026-06-08

- 概览页新增 `一键读取本机设置`，集中展示本机数据位置、备份保存位置、应用配置/历史/日志路径和最近备份记录。
- 一键读取保持只读边界：只读取配置、历史和路径，不执行备份、恢复，不安装、卸载、加载或修改定时任务。
- 顶部运行模式切换从普通界面移除，保留到设置页的 `开发运行模式`，降低普通用户视角里的复杂度。
- 桌面 App 环境下默认进入桌面本机服务模式；浏览器开发环境仍可在设置页切换预览、本机、开发连接和桌面 App 模式。
- GUI/Tauri 版本升至 `0.36.1`。

## 0.36.0 - 2026-06-08

- GUI 做产品降噪：主导航简化为 `概览`、`备份`、`存储位置`、`恢复`、`记录` 和 `设置`。
- 新增独立 `备份` 页，把备份内容、存储位置、加密、保留策略、本地为准一致性备份和最新结果放到同一条日常路径里。
- `存储位置` 页直接展示目标端只读检查结果，用户不需要跳到高级诊断页查看 doctor 输出。
- 首启引导、安装验证、健康检查、macOS 诊断和定时备份状态从主导航移到设置页的高级诊断入口。
- 普通用户界面把 `helper` 概念降噪为 `本机服务`；协议名、代码变量、`helper/server.mjs` 和开发文档中的技术名称继续保留。
- README 和 README_EN 简化桌面 GUI 说明，改为围绕备份、存储位置、完整性、恢复和记录的主流程描述。
- 运行记录中的服务审计标签从 `助手` 改为 `服务`，减少界面里的内部实现术语。
- GUI/Tauri 版本升至 `0.36.0`。
...
- 新增 `buildFirstLaunchGuidance` 纯函数和测试，明确首屏推荐优先级：桌面 App、helper、目标端阻断、只读 doctor、首次真实备份、只读计划状态、健康页。
- 推荐动作只做页面跳转或运行现有只读 doctor，不安装、卸载、加载或修改 `launchd`，不执行真实恢复，不绕过真实备份手动确认。
- GUI 测试覆盖概览页推荐卡片和跳转行为。
- GUI/Tauri 版本升至 `0.35.0`。
